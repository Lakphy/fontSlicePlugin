...
export function unicodeToSubset(unicodes) {
...
export const generateFontSubsetName = (fontFileName, index) => `${fontFileName}.${index + 1}`;
export function createUnicodeRange(list) {
    const result = [];
    const workflow = [];
    const doWork = () => {
...
export function generateCss({ name, fontFamily, fontWeight, fontStyle, fontDisplay, formats, unicodeRange, fontName, }) {
    const src = formats
        .map((format) => {
