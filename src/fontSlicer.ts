...
async function fontSlicer(
  ...
  const extname: string = path.extname(fileName).slice(1);
  if (extname !== "ttf") return; // 只支持ttf
  const font = Font.create(content, { type: extname });
  // @ts-ignore
  const charMap = font.data.cmap;
  const filteredRanges = (unicodeRanges as Array<UnicodeRangeType>)
    .map((item: UnicodeRangeType) => {
